...
extern const uint8_t image_jpg_start[] asm("_binary_test_jpg_start");
extern const uint8_t image_jpg_end[] asm("_binary_test_jpg_end");
...
//显示图片
void s_show_ops(void *in_buf, void **out_buf, size_t buf_size, esp_lcd_panel_handle_t mipi_dpi_panel, ppa_client_handle_t ppa_rotate_handle, void *clear_buf)
{
      // 直接显示原始图像
      ESP_ERROR_CHECK(esp_lcd_panel_draw_bitmap(mipi_dpi_panel, 0, 0, EXAMPLE_IMAGE_W, EXAMPLE_IMAGE_H, in_buf));
    //   vTaskDelay(800 / portTICK_PERIOD_MS);
    //       // 添加永久循环保持显示
    while(1) {
...
void app_main(void)
{
    ...
    // //---------------SHOW------------------//
    ppa_client_handle_t ppa_rotate_handle = NULL;
    ppa_client_config_t ppa_rotate_config = {
        .oper_type = PPA_OPERATION_SRM,
        .max_pending_trans_num = 1,
    };
    ESP_ERROR_CHECK(ppa_register_client(&ppa_rotate_config, &ppa_rotate_handle));
    printf("start show operations\n");
    s_show_ops(pixels, (void **)out_buf, out_buf_size, mipi_dpi_panel, ppa_rotate_handle, clear_buf);
    ...
    ESP_ERROR_CHECK(ppa_unregister_client(ppa_rotate_handle));
